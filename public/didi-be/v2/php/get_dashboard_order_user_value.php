...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = "  select u.name as dated,count(distinct coalesce(m.orderid,'')) as total from x_order_master m
...
group by u.name order by count(distinct coalesce(m.orderid,'')) desc limit 10";
