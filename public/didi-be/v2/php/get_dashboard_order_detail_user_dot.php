	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = "  select u.name as name,sum(coalesce(d.total,0)) as total,count(distinct coalesce(o.orderid,'')) as totalqty from 
						...
						join x_user u on u.usercode in (".$keyword.")
						left join x_order_master o on o.dated=c.dated and o.storecode=m.storecode and o.usercode=u.usercode and iscanceled='0'
