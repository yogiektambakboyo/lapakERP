...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			x.skucode,s.description as skuname,1 as qty,x.skucode||'.jpg' as img,x.min_qty,x.disc_value 
			from x_disc_detail_item x 
			join x_product_sku s on s.skucode=x.skucode  
			join x_user ux on ux.usercode=:usercode and ux.city='KUTA JAYA' and ux.isposadmin='0'
			left join x_product_price p on p.skucode=x.skucode and p.storecode=:storecode
			where x.disc_id='FF-500x' ";
			...
			while ($row = $stmt->fetch()) {
					$skucode = $row["skucode"];
					$barcode = $row["min_qty"].'#'.$row["disc_value"];
					$skuname = $row["skuname"];
					$price = $row["price"];
					$qty = $row["qty"];
					$img = $row["img"];
					$arr[]=array("kode"=>$skucode,"barcode"=>$barcode,"nama"=>$skuname,"harga"=>$price,"gambar"=>$img);
				}	
		}
	}
