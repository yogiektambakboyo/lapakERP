...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = "  select s.printdesc as dated,sum(coalesce(d.total,0)) as total from x_order_master m
						...
						group by s.printdesc order by sum(coalesce(d.total,0)) desc limit 10";
