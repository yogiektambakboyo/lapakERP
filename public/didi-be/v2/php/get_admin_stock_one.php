...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			$arr=array("pricem1"=>$pricem1,"pricem2"=>$pricem2,"skucode"=>$skucode,"storecode"=>$storecode,"skuname"=>$skuname,"price"=>$price,"brandcode"=>$brandcode,"categorycode"=>$categorycode,"subcategorycode"=>$subcategorycode,"casesize"=>$casesize,"printdesc"=>$prindesc);
		}else{
			//storecode,skucode,skuname,price,brandcode,categorycode,subcategorycode,casesize
			$query = "select COALESCE(p.pricemember_1,'0') as pricem1,COALESCE(p.pricemember_2,'0') as pricem2,p.storecode,s.skucode,s.description as skuname,c.categoryname as categorycode,b.brandname as brandcode,g.subcategoryname as subcategorycode,s.casesize,CAST(p.price as INTEGER) price,s.printdesc 
