...
//if($method == 'SFA_Borwita_Android'){
	try {
	   ...
	   if (!$dbh) {
			...
			if(intval($c)>0){
				$query = " UPDATE x_orderdraft set qty=qty+(:qty),price=:price,sequence=:sequence where usercode=:usercode and storecode=:storecode and skucode=:skucode  and isinvoice='0'  ";
				$stmt = $dbh->prepare($query);
				$stmt->bindParam(':usercode', $usercode, PDO::PARAM_STR,50);
				$stmt->bindParam(':storecode', $storecode, PDO::PARAM_STR,50);
				$stmt->bindParam(':skucode', $skucode, PDO::PARAM_STR,50);
				...
				$query = " INSERT INTO x_orderdraft(usercode, storecode, skucode, qty, price, sequence, createdate,isinvoice) select usercode,storecode, :skucode, :qty, :price, :sequence, now(),'0' from x_user where usercode=:usercode2 ";
				$stmt = $dbh->prepare($query);
				$stmt->bindParam(':usercode', $usercode, PDO::PARAM_STR,50);
				$stmt->bindParam(':usercode2', $usercode, PDO::PARAM_STR,50);
				$stmt->bindParam(':storecode', $storecode, PDO::PARAM_STR,50);
