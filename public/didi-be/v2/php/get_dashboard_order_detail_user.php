...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = "  select c.dated,b.name as name,sum(coalesce(d.total,0)) as total from 
						x_calendar c 
						join x_store_master m on m.storecode=:storecode
						join x_user b on b.usercode in (".$keyword.")
						left join x_order_master o on o.dated=c.dated and o.storecode=m.storecode and o.usercode=b.usercode and iscanceled='0'
						left join x_order_detail d on d.orderid=o.orderid
						where c.dated between :begindate and :enddate
						group by c.dated,b.name
						order by dated ";
