	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = " select count(1) as counter from x_order_master o
					...
					join x_disc_detail_item i on i.disc_id='FF-130' and i.skucode=s.skucode
			...
			if(intval($cost)>0){
				...
				$query = " select count(1) as counter from x_disc_master where disc_id='FF-130'  and  CURRENT_DATE between start_date and end_date "; 
				$stmt = $dbh->prepare($query);		
				$stmt->execute();			
				while ($row = $stmt->fetch()) {
					$cost = $row["counter"];
