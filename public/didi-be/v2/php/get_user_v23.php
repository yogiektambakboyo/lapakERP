...
//if($method == 'SFA_Borwita_Android'){
	$city="-";$username="-";$password="-";$name="-";$storecode="-";$usercode="-";$userrealname="-";$useraddress="-";$longitude="0";$latitude="0";
	try {
	   $dbh = new PDO("pgsql:dbname=didi;host=10.17.206.43", 'postgres', 'postgres'); 
	   if (!$dbh) {
			$arr=array("city"=>$city,"username"=>$username,"password"=>$password,"store"=>$name,"storecode"=>$storecode,"usercode"=>$usercode,"userrealname"=>$userrealname,"useraddress"=>$useraddress,"longitude"=>$longitude,"latitude"=>$latitude);
		}else{
			...
			$query = "select city,username,password,name,storecode,usercode,address,longitude,latitude from x_user where username=:username and password=:password and active='1' ";
			$stmt = $dbh->prepare($query);
