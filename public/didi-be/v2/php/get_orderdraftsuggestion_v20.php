	   ...
	   if (!$dbh) {
			...
			if(intval($isfromdidi)>0){
				$query = " select coalesce(i.disc_value,0) as disc_value,x.skucode,x.storecode,s.description as skuname,
						CASE WHEN u.member_type=1 THEN coalesce(p.pricemember_1,0)+((coalesce(p.pricemember_1,0)*f.upvalue)/100) 
						WHEN u.member_type=2 THEN coalesce(p.pricemember_2,0)+((coalesce(p.pricemember_2,0)*f.upvalue)/100) 
						ELSE CAST((COALESCE(p.price,0)+((coalesce(p.price,0)*f.upvalue)/100)) as numeric(14,0)) END as price,
				...
				$query = " select coalesce(i.disc_value,0) as disc_value,x.skucode,x.storecode,s.description as skuname,
CASE WHEN u.member_type=1 THEN coalesce(p.pricemember_1,0)+((coalesce(p.pricemember_1,0)*g.upvalue)/100) 
WHEN u.member_type=2 THEN coalesce(p.pricemember_2,0)+((coalesce(p.pricemember_2,0)*g.upvalue)/100) 
ELSE CAST((COALESCE(p.price,0)+((coalesce(p.price,0)*g.upvalue)/100)) as numeric(14,0)) END as price,
x.qty,x.skucode||'.jpg' as img 
from x_orderdraft x 
join x_user u on u.usercode=x.usercode 
