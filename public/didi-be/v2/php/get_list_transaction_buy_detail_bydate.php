...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = "  select 
						d.skucode,x.storecode,s.description as skuname,coalesce(d.price,0) as price,d.pcsqty as qty,d.skucode||'.jpg' as img,dated,x.orderid,d.total
						from x_order_master x
						join x_order_detail d on d.orderid=x.orderid
						join x_user u on u.usercode=u.usercode
						join x_product_sku s on s.skucode=d.skucode
						join x_partner p on p.handphone=u.phoneno
						where x.usercode=:usercode and x.iscanceled='0' and x.dated between :begindate and :enddate  ";
