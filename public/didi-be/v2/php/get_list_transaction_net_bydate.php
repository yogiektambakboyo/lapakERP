...
if($method == 'DIDI_POS_Mobile'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = "  select u.name,m.orderid,dated,
						m.total-(sum(coalesce(s.disc_value,0)+coalesce((pp.price_hpp*d.pcsqty),0))) as total,
						sum(d.pcsqty) as qty,case when iscanceled='1' THEN 'BATAL' when iscanceled='0' AND isprint='0' AND a.jml<=1 and m.uploadtime>=now()::timestamp- interval '10 minutes' THEN 'TERPROSES' when iscanceled='0' AND isprint='0' AND sum(a.jml)>1 THEN 'ANTRI' WHEN iscanceled='0' AND isprint='1' THEN 'TERCETAK' ELSE 'ANTRI' END as status 
						...
						where m.storecode=:storecode2 and m.dated between :begindate and :enddate
