...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = "  select m.dated,CAST(sum(d.total) as INT) as total from x_order_master m join x_order_detail d on d.orderid=m.orderid where m.storecode=:storecode 
and m.dated between :begindate and :enddate and iscanceled='0'  group by m.dated  
order by m.dated ";
