...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = " select id,usercode,nominal,isapproved,coalesce(file_attachment,'-') file_attachment,to_char(created_date,'YYYY-MM-DD') created_date from x_deposit_order d where usercode=:user order by created_date desc ";
			$stmt = $dbh->prepare($query);
