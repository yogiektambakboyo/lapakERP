...
//if($method == 'SFA_Borwita_Android'){
	...
	try {
	   ...
	   if (!$dbh) {
			...
			$query = "   select x.isprint,f.name as storename,f.address,f.city,d.skucode,x.storecode,s.printdesc as skuname,coalesce(d.price,0) as price,d.pcsqty as qty,dated,x.orderid,d.total,x.picktime,COALESCE(x.remarks,'-') as remarks,x.total as totalinv,u.name
						 ...
						 where x.orderid=:orderid and x.iscanceled='0' ";
			$stmt = $dbh->prepare($query);
			...
			while ($row = $stmt->fetch()) {
					...
					$address = $row["address"];
					$isprint = $row["isprint"];
					$arr[]=array("storename"=>$storename,"address"=>$address,"city"=>$city,"skucode"=>$skucode,"storecode"=>$storecode,"skuname"=>$skuname,"price"=>$price,"qty"=>$qty,"dated"=>$dated,"orderid"=>$orderid,"total"=>$total,"picktime"=>$picktime,"remarks"=>$remarks,"totalinv"=>$totalinv,"name"=>$name,"isprint"=>$isprint);
				}	
		}
	}
