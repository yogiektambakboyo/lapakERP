...
class TerapistController extends Controller
{
    ...
    public function index(Request $request) 
    {
        ...
        $worktime = DB::select("
                select u.active,u.id,u.name,u.join_date,u.work_year,u.gender,u.netizen_id,u.address,u.phone_no,
        u.level_up_date,u.referral_id, max(id.created_at::date) as last_activity, string_agg(distinct b.remark,', ') as branch_name
        from users u 
        join users_branch ub  on ub.user_id = u.id 
        join branch b  on b.id = ub.branch_id 
        left join invoice_detail id on id.assigned_to = u.id 
        where u.job_id = 2 and ub.branch_id in (select ub2.branch_id from users_branch ub2 where ub2.user_id = ".$user->id."   )
        group by u.id,u.name,u.phone_no,u.join_date,u.work_year,u.gender,u.netizen_id,u.address,u.level_up_date,u.referral_id
        order by 1 desc,3 asc

");
        return view('pages.terapist.index',['company' => Company::get()->first()], compact('worktime_sum','filter_end_date','filter_begin_date','leave','worktime','data','keyword','act_permission','branchs','payment_type'))->with('i', ($request->input('page', 1) - 1) * 5);
    }
    ...
    public function search(Request $request) 
    {
        ...
        return view('pages.terapist.index',['company' => Company::get()->first()], compact('worktime','data','keyword','act_permission','branchs'))->with('i', ($request->input('page', 1) - 1) * 5);
        }
    }
