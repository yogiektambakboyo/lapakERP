...
class LeaveController extends Controller
{
    ...
    public function index(Request $request) 
    {
        ...
        $worktime = DB::select("select wt.id,b.remark as branch_name,u.name as nama,wt.dated_start,wt.dated_end,wt.reason,to_char(wt.created_at,'dd-MM-YYYY HH24:MI') created_at,coalesce(wt.is_approve,0) is_approve
        from leave_request  wt 
        join users u on u.id = wt.user_id 
        join users_branch ub  on ub.user_id = u.id 
        join branch b  on b.id = ub.branch_id 
        where ub.branch_id in (select ub2.branch_id from users_branch ub2 where ub2.user_id = ".$user->id."  ) and wt.dated_start>now()-interval'67 day'
        order by wt.id");
        return view('pages.leave.index',['company' => Company::get()->first()], compact('worktime','data','keyword','act_permission','branchs','payment_type'))->with('i', ($request->input('page', 1) - 1) * 5);
    }
    ...
    public function search(Request $request) 
    {
        ...
        if($request->export=='Export Excel'){
            ...
            where ub.branch_id in (select ub2.branch_id from users_branch ub2 where ub2.user_id = ".$user->id." ) and wt.dated_start between '".$begindate."' and '".$enddate."'
            order by wt.id");
