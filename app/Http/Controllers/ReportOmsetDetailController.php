...
use App\Exports\ReportOmsetDetailExport;
...
class ReportOmsetDetailController extends Controller
{
    /**
    ...
    private $data,$act_permission,$module="reports.omsetdetail",$id=1;
    ...
    public function search(Request $request) 
    {
        ...
        if($request->export=='Export Excel'){
            ...
            return Excel::download(new ReportOmsetDetailExport($strencode), 'report_omset_detail_'.Carbon::now()->format('YmdHis').'.xlsx');
        }else{
            $report_data = DB::select("
                select b.remark as branch_name,im.dated,im.invoice_no,id.product_name,pc.remark as category_name,id.qty,id.uom,id.total as total,id.price_purchase,case when coalesce(id.price_purchase)<=0 then 0 else id.total-id.price_purchase  end as margin
                from invoice_master im 
