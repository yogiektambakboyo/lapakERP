...

{
...

    {
		...
		$data = $results;
		return view('master_user',["menu_data"=>$data]);
    }
	...
	public function getDataRange($tim_pe)
    {
		...
		$results = DB::select("select u.storecode,u.longitude,u.latitude,u.member_type,u.isneedprint,u.isposadmin,u.issuspend,u.usercode,u.name,u.address,u.city,u.phoneno,u.username,'--------' as password,u.createdate,u.active,m.name as wholesaler 
from x_user u
join x_store_master m on m.storecode=u.storecode where u.city like :tim_pe ", ['tim_pe' => $tim_pe]);
		return DataTables::collection($results)->toJson();		
        //return Datatables::of(User::query())->make(true);
    }
