...
use App\Exports\ReportReferralExport;
...
class ReportReferralController extends Controller
{
    /**
    ...
    private $data,$act_permission,$module="reports.referral",$id=1;
    ...
    public function index(Request $request) 
    {
        ...
        $report_data = DB::select("
            ...
            join users_branch ub on ub.user_id = u.id and ub.branch_id in (select branch_id from users_branch where user_id=".$user->id." )
        ...
        return view('pages.reports.referral',['company' => Company::get()->first()], compact('shifts','branchs','data','keyword','act_permission','report_data'))->with('i', ($request->input('page', 1) - 1) * 5);
    }
    ...
    public function search(Request $request) 
    {
        ...
        if($request->export=='Export Excel'){
            ...
            return Excel::download(new ReportReferralExport($strencode), 'report_referral_'.Carbon::now()->format('YmdHis').'.xlsx');
        }else{
            $report_data = DB::select("
                select b.remark as branch_name,u.employee_id,u.name,u.join_date,u.join_years,uf.name as referral_name
                from users u
