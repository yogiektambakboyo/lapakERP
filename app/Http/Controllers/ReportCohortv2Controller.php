...
class ReportCohortv2Controller extends Controller

{
...

    {
		...
		$data = $results;
		return view('report_cohort_v2',["menu_data"=>$data]);
    }
    ...
    date_trunc('week', U.createdate)::date as cohort_month,
    usercode
...
where B.cohort_month IS NOT NULL order by 1, 3) a");
	...
	from x_calendar c
	left join x_user u on date_trunc('week',u.createdate)::date=date_trunc('week',c.dated)::date AND u.city not in ('DEMO','DUMMY','LAMONGAN','SYSTEM','X-AREA','SURABAYA DUMMY')
	group by c.dated
	) a 
left join
