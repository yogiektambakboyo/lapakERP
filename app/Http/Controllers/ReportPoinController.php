...
class ReportPoinController extends Controller

{
...

    {
		...
		$data = $results;
		return view('report_poin',["menu_data"=>$data]);
    }
    ...
    {
		$results = DB::select(" select u.usercode,u.name,u.address,d.poin as nominal from x_poin d 
join x_user u on u.usercode=d.usercode");
    ...
    {
		$results = DB::select(" select id,description,poin as nominal,to_char(created_date,'YYYY-MM-DD') as created_date from x_poin_history where storecode=:usercode order by created_date desc",['usercode'=>$usercode]);
		return DataTables::collection($results)->toJson();		
        //return Datatables::of(User::query())->make(true);
    }
