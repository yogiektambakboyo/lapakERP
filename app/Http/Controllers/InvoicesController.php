...
class InvoicesController extends Controller
{
    ...
    public function print(Invoice $invoice) 
    {
        ...
        /**return view('pages.invoices.print', [
            ...
            'invoiceDetails' => InvoiceDetail::join('invoice_master as om','om.invoice_no','=','invoice_detail.invoice_no')->join('product_sku as ps','ps.id','=','invoice_detail.product_id')->join('product_uom as u','u.product_id','=','invoice_detail.product_id')->join('uom as um','um.id','=','u.uom_id')->join('customers as c','c.id','=','om.customers_id')->join('branch as bc','bc.id','=','c.branch_id')->where('invoice_detail.invoice_no',$invoice->invoice_no)->orderByRaw('ps.type_id ASC,invoice_detail.seq  ASC')->get(['bc.remark as branch_name','bc.address as branch_address','um.remark as uom','invoice_detail.qty','invoice_detail.price','invoice_detail.total','ps.id','ps.remark as product_name','invoice_detail.discount','om.tax','om.voucher_code']),
        ]);**/
    ...
    public function printsj(Invoice $invoice) 
    {
        ...
        $data = $this->data;
        $users = User::join('users_branch as ub','ub.branch_id', '=', 'users.branch_id')->where('ub.user_id','=',$user->id)->get(['users.id','users.name']);
