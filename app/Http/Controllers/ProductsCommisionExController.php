...
use App\Models\ProductCommisionsEx;
use App\Models\ProductCommisions;
...
class ProductsCommisionExController extends Controller
{
    /**
    ...
    private $data,$act_permission,$module="productscommision",$id=1;
    ...
    public function index(Request $request) 
    {
        ...
        $products = Product::orderBy('product_sku.remark', 'ASC')
                    ...
                    ->join('product_commisions_ex as pr','pr.product_id','=','product_sku.id')
                    ->join('branch as bc','bc.id','=','pr.branch_id')
                    ->join('users_branch as ub2','ub2.branch_id', '=', 'pr.branch_id')
                    ->leftJoin('users as us','us.id', '=', 'pr.users_id')
                    ->where('ub2.user_id','=',$user->id)
                    ->where('pt.id','=','1')
                    ->get(['created_by_fee','pr.users_id', 'us.name as pic_name', 'referral_fee','product_sku.id','product_sku.remark as product_name','pr.branch_id','bc.remark as branch_name','pb.remark as product_brand']);
        return view('pages.productscommisionex.index', ['company' => Company::get()->first()],compact('products','data','keyword','act_permission'))->with('i', ($request->input('page', 1) - 1) * 5);
    }
    ...
    public function create() 
    {
        ...
        $user  = Auth::user();
        $data = $this->data;
        return view('pages.productscommisionex.create',[
            'products' => DB::select('select ps.id,ps.remark from product_sku as ps where ps.type_id=1 order by remark;'),
            'data' => $data, 'company' => Company::get()->first(), 'users' => User::where('users.job_id','=',1)->orderBy('users.name','ASC')->get(['users.id','users.name']),
     ...
     * 
     * @param ProductCommisionsEx $productcommision
     * @param Request $request
     * 
     * @return \Illuminate\Http\Response
     */
    public function store(ProductCommisionsEx $productcommision, Request $request) 
    {
        ...
        return redirect()->route('productscommision_ex.index')
    ...
    public function show(Product $product) 
    {
        ...
        return view('pages.productscommisionex.show', [
            'product' => $products ,
    ...
    public function edit(String $branch_id,String $product_id) 
    {
        ...
        ->get(['created_by_fee', 'pr.users_id', 'referral_fee','product_sku.id as id','product_sku.abbr','product_sku.brand_id','product_sku.category_id','product_sku.type_id','product_sku.remark as product_name','pr.branch_id','bc.remark as branch_name'])->first();
        return view('pages.productscommisionex.edit', [
            'branchs' => Branch::join('users_branch as ub','ub.branch_id','=','branch.id')->where('ub.user_id','=',$user->id)->get(['branch.id','branch.remark']),
            'data' => $data,
            'product' => $product, 'company' => Company::get()->first(),
            'users' => User::where('users.job_id','=',1)->orderBy('users.name','ASC')->get(['users.id','users.name']),
     ...
     * Update user data
     * 
     * @param ProductCommisions $product
     * @param Request $request
     * 
     * @return \Illuminate\Http\Response
     */
     ...
     * Delete user data
     * 
     * @param ProductCommisions $productcommisions
     * 
     * @return \Illuminate\Http\Response
     */
    public function destroy(String $branch,String $product) 
    {
        ProductCommisionsEx::where('product_id','=',$product)->where('branch_id','=',$branch)->delete();
        return redirect()->route('productscommision_ex.index')
