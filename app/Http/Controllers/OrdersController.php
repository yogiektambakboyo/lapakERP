...
class OrdersController extends Controller
{
    ...
    public function getproduct() 
    {
        ...
        $data = $this->data;
        $user = Auth::user();
        $product = DB::select("select product_sku.barcode,'' as executed_at,m.remark as uom,product_sku.vat as vat_total,product_sku.id,product_sku.remark,product_sku.abbr,pt.remark as type,pc.remark as category_name,pb.remark as brand_name,pp.price+coalesce(pa.value,0) as price,'0' as discount,'0' as qty,'0' as total
        from product_sku
    ...
    public function print(Order $order) 
    {
        ...
        $pdf = PDF::setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true])->loadView('pages.orders.print', [
            'data' => $data,
            'customers' => Customer::join('order_master as om','om.customers_id','customers.id')->join('branch as b','b.id','=','customers.branch_id')->get(['b.remark as branch_name','customers.id','customers.name']),
            'branchs' => Branch::join('users_branch as ub','ub.branch_id', '=', 'branch.id')->where('ub.user_id','=',$user->id)->get(['branch.id','branch.remark']),
            'users' => $users,
