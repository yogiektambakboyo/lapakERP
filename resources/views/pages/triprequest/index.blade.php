...
@section('title', 'Rekap Perjalanan')

@section('content')
    <div class="bg-light p-4 rounded">
        <h1>Rekap Perjalanan</h1>
        <div class="lead row mb-3">
            <div class="col-md-10">
                ...
                <br>
                <div class="col-md-8"> 	
                    <form action="{{ route('triprequest.search') }}" method="POST" class="row row-cols-lg-auto g-3 align-items-center">
                        @csrf
            ...
            <div class="col-md-2">
                <a href="{{ route('triprequest.create') }}" class="btn btn-primary float-right {{ $act_permission->allow_create==1?'':'d-none' }}"><span class="fa fa-circle-plus"></span> @lang('general.btn_create')</a>
            </div>
        </div>
        ...
        <table class="table table-striped" id="example">
            ...
            <tr>
                ...
                <th scope="col" width="8%">No Dokumen</th>
                <th scope="col" width="8%">Nama</th>
                <th scope="col" width="8%">Tgl Awal</th>
                <th scope="col" width="8%">Tgl Akhir</th>
                <th scope="col" width="12%">Cabang Asal</th>
                <th scope="col" width="12%">Cabang Tujuan</th>
            ...
            <tbody>
                ...
                @foreach($doc as $purchase)
                    <tr>
                        <td>{{ $purchase->branch_name }}</td>
                        <td>{{ $purchase->doc_no }}</td>
                        <td>{{ $purchase->name }}</td>
                        <td>{{ Carbon\Carbon::parse($purchase->dated_start)->format('d-m-Y') }}</td>
                        <td>{{ Carbon\Carbon::parse($purchase->dated_end)->format('d-m-Y') }}</td>
                        <td>{{ $purchase->location_source }}</td>
                        <td>{{ $purchase->location_destination }}</td>
                        <td>{{ number_format($purchase->total,0,',','.') }}</td>
                        <td><a href="{{ route('triprequest.show', $purchase->id) }}" class="btn btn-warning btn-sm  {{ $act_permission->allow_show==1?'':'d-none' }}">@lang('general.lbl_show')</a></td>
                        <td><a href="{{ route('triprequest.edit', $purchase->id) }}" class="btn btn-info btn-sm  {{ $act_permission->allow_edit==1?'':'d-none' }} ">@lang('general.lbl_edit')</a></td>
                        <td class=" {{ $act_permission->allow_delete==1?'':'d-none' }}">
                            <a onclick="showConfirm({{ $purchase->id }}, '{{ $purchase->doc_no }}')" class="btn btn-danger btn-sm  {{ $act_permission->allow_delete==1?'':'d-none' }} ">@lang('general.lbl_delete')</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <div class="modal fade" id="modal-filter" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('triprequest.search') }}" method="POST">   
                        @csrf 
                        <div class="col-md-10">
          ...
          function showConfirm(id,data){
            ...
            confirmButtonText: "@lang('general.lbl_sure_delete')"
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('triprequest.destroy','XX') }}";
                    var lastvalurl = "XX";
                    console.log(url);
