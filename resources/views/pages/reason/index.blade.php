    ...
    <div class="bg-light p-4 rounded">
        ...
        <div class="lead row mb-3">
            <div class="col-md-10">
                ...
                <div class="col-md-10"> 	
                    <form action="{{ route('reason.search') }}" method="GET" class="row row-cols-lg-auto g-3 align-items-center">
            ...
            <div class="col-md-2">
                <a href="{{ route('reason.create') }}" class="btn btn-primary float-right  {{ $act_permission->allow_create==1?'':'d-none' }}"><span class="fa fa-plus-circle"></span>  @lang('general.btn_create')</a>
            </div>
        </div>
        ...
        <table class="table table-striped" id="example">
            ...
            <tbody>
                ...
                @foreach($doc_data as $product)
                    <tr>
                        <td>{{ $product->reason_type }}</td>
                        <td>{{ $product->remark }}</td>
                        <td><a href="{{ route('reason.edit', [$product->id]) }}" class="noexport btn btn-info btn-sm  {{ $act_permission->allow_edit==1?'':'d-none' }}">@lang('general.lbl_edit')</a></td>
                        <td class="noexport {{ $act_permission->allow_delete==1?'':'d-none' }}">
                            <a onclick="showConfirm( '{{ $product->id }}','{{ $product->remark }}' )" class="btn btn-danger btn-sm  {{ $act_permission->allow_delete==1?'':'d-none' }} ">@lang('general.lbl_delete')</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <div class="modal fade" id="modal-filter" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('reason.search') }}" method="GET">   
                        @csrf 
                        <div class="col-md-10">
    ...
    <script type="text/javascript">
        ...
        function showConfirm(id,data){
            ...
            if (result.isConfirmed) {
                var url = "{{ route('reason.destroy',['AA']) }}";
                var lastvalurl = "AA";
