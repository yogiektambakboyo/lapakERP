...
@section('content')
<form method="POST" action="{{ route('petty.store') }}"  enctype="multipart/form-data">
  @csrf
  <div class="panel text-white">
    <div class="panel-heading  bg-teal-600">
      <div class="panel-title"><h4 class="">@lang('general.lbl_petty') #{{ $invoice->doc_no }}</h4></div>
      <div class="">
        <a href="{{ route('petty.index') }}" class="btn btn-default">@lang('general.lbl_cancel')</a>
      ...
      var sub_total = 0;
        ...
        $('#save-btn').on('click',function(){
          if($('#dated').val()==''){
            ...
            if(counterBlank>0){
              ...
              const res = axios.patch("{{ route('petty.update',$invoice->id) }}", json, {
                headers: {
        ...
        function addProduct(id,abbr, price, qty, uom,vat_total,total,type){
          ...
          $("#apply-voucher-btn").on('click',function(){
            ...
            //Get Invoice 
            const resInvoice = axios.get("{{ route('petty.getinvoice',$invoice->doc_no) }}", {
              headers: {
                // Overwrite Axios's automatically set Content-Type
                'Content-Type': 'application/json'
              }
