    ...
    <div class="bg-light p-4 rounded">
        ...
        <div class="lead row mb-3">
            <div class="col-md-10">
                ...
                <div class="col-md-10"> 	
                    <form action="{{ route('promo.search') }}" method="GET" class="row row-cols-lg-auto g-3 align-items-center">
            ...
            <div class="col-md-2">
                <a href="{{ route('promo.create') }}" class="btn btn-primary float-right  {{ $act_permission->allow_create==1?'':'d-none' }}"><span class="fa fa-plus-circle"></span>  @lang('general.btn_create')</a>
            </div>
        </div>
            ...
            <tbody>
                ...
                @foreach($products as $product)
                    <tr>
                        ...
                        <td>{{ Carbon\Carbon::parse($product->date_start)->format('d-m-Y') }}</td>
                        <td>{{ Carbon\Carbon::parse($product->date_end)->format('d-m-Y') }}</td>
                        <td>{{ number_format($product->value,0,',','.') }}</td>
                        <td>{{ number_format($product->value_idx,0,',','.') }}</td>
                        <td><a href="{{ route('promo.edit', [$product->branch_id,'0',$product->date_start,$product->date_end,$product->promo_id]) }}" class="noexport btn btn-info btn-sm  {{ $act_permission->allow_edit==1?'':'d-none' }}">@lang('general.lbl_edit')</a></td>
                        <td class="noexport {{ $act_permission->allow_delete==1?'':'d-none' }}">
                            <a onclick="showConfirm( '{{ $product->branch_id }}','0','{{ $product->date_start }}','{{ $product->date_end }}','{{ $product->promo_id }}' )" class="btn btn-danger btn-sm  {{ $act_permission->allow_delete==1?'':'d-none' }} ">@lang('general.lbl_delete')</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <div class="modal fade" id="modal-filter" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('promo.search') }}" method="GET">   
                        @csrf 
                        <div class="col-md-10">
    ...
    <script type="text/javascript">
        ...
        function showConfirm(branch_id,product_id,dated_start,dated_end,data){
            ...
            if (result.isConfirmed) {
                var url = "{{ route('promo.destroy',['AA','BB','CC','DD','EE']) }}";
                var lastvalurl = "AA";
                var lastvalurl2 = "BB";
                var lastvalurl3 = "CC";
                var lastvalurl4 = "DD";
                        ...
                        if(resp.data.status=="success"){
                            Swal.fire({
                                ...
                                confirmButtonText: "@lang('general.lbl_close') "
                                    ...
                                    window.location.href = "{{ route('promo.index') }}"; 
                                })
                        }else{
                            Swal.fire(
                            {
