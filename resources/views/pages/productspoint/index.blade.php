...
@section('content')
    <div class="bg-light p-4 rounded">
        <h1>Products Point</h1>
        <div class="lead row mb-3">
            <div class="col-md-10">
                <div class="col-md-4">
                    Manage your products point here.
                </div>
                <div class="col-md-10"> 	
                    <form action="{{ route('productspoint.search') }}" method="GET" class="row row-cols-lg-auto g-3 align-items-center">
            ...
            <div class="col-md-2">
                <a href="{{ route('productspoint.create') }}" class="btn btn-primary float-right  {{ $act_permission->allow_create==1?'':'d-none' }}"><span class="fa fa-plus-circle"></span>  Add new product point</a>
            </div>
        </div>
            ...
            <tbody>
                ...
                @foreach($products as $product)
                    <tr>
                        ...
                        <td>{{ $product->point }}</td>
                        <td><a href="{{ route('productspoint.edit', [$product->branch_id,$product->id]) }}" class="btn btn-info btn-sm  {{ $act_permission->allow_edit==1?'':'d-none' }}">Edit</a></td>
                        <td class="{{ $act_permission->allow_delete==1?'':'d-none' }}">
                            {!! Form::open(['method' => 'DELETE','route' => ['productspoint.destroy', [$product->branch_id,$product->id]],'style'=>'display:inline']) !!}
