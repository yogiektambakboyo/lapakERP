...
@section('title', 'Laporan - Rating & Review')
...
@section('content')
    <div class="bg-light p-4 rounded">
        ...
        <table class="table table-striped" id="example">
            <thead>
            <tr>
                <th scope="col" width="10%">Cabang</th>
            ...
            <tbody>
                ...
                @foreach($report_data as $rdata)
                    <tr>
                        ...
                        <th scope="row">{{ $rdata->invoice_no }}</th>
                        <td>{{ Carbon\Carbon::parse($rdata->invoice_date)->format('d-m-Y') }}</td>
                        <td>{{ $rdata->customers_name }}</td>
                        <td>{{ $rdata->name }}</td>
                        <td>{{ $rdata->value_review }}</td>
                        <td>{{ $rdata->remarks }}</td>
        ...
        <div class="modal fade" id="modal-filter2" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('reports.review.search') }}" method="GET">   
                        @csrf 
                        <div class="col-md-10">
