...
@section('title', 'Purchase Order Internal')

@section('content')
    <div class="bg-light p-4 rounded">
        <h1>@lang('general.lbl_purchase_internal')</h1>
        ...
        <div class="modal fade" id="modal-filter" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('purchaseordersinternal.search') }}" method="POST">   
                        @csrf 
                        <div class="col-md-10">
        ...
        <div class="modal fade" id="modal-status" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">@lang('general.lbl_status')</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <label class="form-label col-form-label col-md-8" id="product_id_selected_lbl">@lang('general.lbl_statusselect') </label>
                  <div class="col-md-8">
                    <select class="form-control" 
                ...
                <button type="button" class="btn btn-primary"  data-bs-dismiss="modal" id="btn_change_status">@lang('general.lbl_apply')</button>
                </div>
            </div>
            </div>
          </div>
    ...
    <script type="text/javascript">
          ...
          $('#btn_change_status').on('click', function(){
            const json = JSON.stringify({
                id : g_id,
                remark : $('#status option:selected').val()
              });
            var url = "{{ route('purchaseordersinternal.updatestatus','XX') }}";
                    var lastvalurl = "XX";
                    console.log(url);
                    url = url.replace(lastvalurl, g_id)
                    const res = axios.patch(url, json, {
                        headers: {
                            ...
                            resp => {
                                if(resp.data.status=="success"){
                                    Swal.fire({
                                        title: 'Updated!',
                                        text: "@lang('general.lbl_msg_update_title') ",
                                        icon: 'success',
          ...
          function showConfirm(id,data){
            ...
            confirmButtonText: "@lang('general.lbl_sure_delete')"
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('purchaseordersinternal.destroy','XX') }}";
                    var lastvalurl = "XX";
                    console.log(url);
